...
Among the patterns that emerge consistently across both primary and subcategory levels, none is more striking or more practical than the divergence between campaign volume and success rate. Technology (66.7%), Photography (61.9%), and Publishing (59.7%) lead at the primary category level but do not rank among the three most popular categories by volume. That distinction belongs to Theater, Film & Video, and Music, which succeed at or below the overall average yet together account for nearly 70% of all campaigns.

The subcategory results broadly reinforce the primary category pattern. The highest-performing subcategories — Web and Technology (70.6%), Translations and Publishing (66.7%), Television within Film and Video (64.7%), and Wearables within Technology (62.2%) — are drawn predominantly from primary categories that already rank toward the top of the success rate distribution, suggesting that primary category selection carries meaningful predictive weight at the subcategory level as well.

The more instructive finding, however, is where this pattern breaks down. Television outperforms its parent category, Film and Video, by 7.4 percentage points despite Film and Video's mid-range standing. This divergence indicates that subcategory positioning is not simply inherited from the primary category — a subcategory can carve out a meaningfully stronger success profile than its parent through factors specific to its own competitive dynamics, audience, or project type. Television's outperformance suggests that projects in that subcategory may benefit from clearer audience targeting, stronger backer familiarity with the format, or simply less competition relative to the broader film and video pool.

Taken together, the subcategory data supports a two-level view of category selection: primary category establishes a baseline expectation for success, but subcategory choice can either reinforce or meaningfully revise that expectation — and in cases like Television, the revision can be substantial.
...
These two distributional profiles stand in direct contrast: high goal variance and low pledge variance characterize failed and canceled campaigns while low goal variance and high pledge variance characterize successful campaigns. Considered jointly, the standard deviation figures across all outcome groups support an interpretation in which goal discipline functions as a structural antecedent of success rather than a mere correlate, a condition that precedes and enables favorable outcomes rather than simply accompanying them.
